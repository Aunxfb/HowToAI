...
Subagent activity is surfaced in the ChatGPT desktop app, Codex CLI, and the IDE
  extension.

Codex spawns subagents when you ask directly or when applicable AGENTS.md or
skill instructions request delegation. At the Ultra intelligence level, Codex
can also delegate proactively when parallel agents would materially improve
speed or quality. Because each subagent does its own model and tool work,
...
Codex spawns a new agent when asked directly, when AGENTS.md or skill instructions request delegation, or (at Ultra level) proactively when it would improve results.
...
| `agents.max_concurrent_threads_per_session` | number |    No  | Cap concurrently open spawned-agent threads (excl. primary). |
...
- `agents.enabled` defaults to `true`. Set to `false` to disable multi-agent tools.
- `agents.max_concurrent_threads_per_session` — when unset, Codex chooses the default. Existing configs can use `agents.max_threads` as a legacy alias.
- `agents.default_subagent_model` and `agents.default_subagent_reasoning_effort` let you set defaults for spawned agents. Explicit spawn values override these.
- `agents.interrupt_message` defaults to `true`. Set to `false` to omit the interruption message from context.
...
If you don't pin a model or `model_reasoning_effort`, Codex can choose a setup that balances intelligence, speed, and price for the task. When you want finer control, set `model` and `model_reasoning_effort` directly in the agent file.

| Model              | Best for                                                              |
| ------------------ | --------------------------------------------------------------------- |
| `gpt-5.6`          | Demanding, multi-step work needing planning, tool use, and validation |
| `gpt-5.4`          | Balanced coding and reasoning workflows                              |
| `gpt-5.6-terra`    | Speed- and efficiency-focused tasks (exploration, read-heavy scans)   |

| Reasoning effort   | When to use                                                          |
| ------------------ | -------------------------------------------------------------------- |
| `ultra`            | Deepest reasoning (model-dependent)                                  |
| `high`             | Complex logic, security review, edge-case analysis                   |
| `medium`           | Balanced default for most agents                                     |
| `low`              | Straightforward, speed-critical tasks                                |
...
- `agents.max_concurrent_threads_per_session` caps how many agent threads can stay open concurrently. `agents.max_threads` works as a legacy alias.
