...
> Complete reference for agent configuration options in OpenCode.

## Overview

This document covers all configuration options for OpenCode agents, including JSON config, markdown agent files, permissions, model selection, and advanced options. It is the deep reference companion to the [agent overview](opencode-agents.md).
...
```json title="opencode.json" [Copy-Safe]
{
...
```markdown title="~/.config/opencode/agents/review.md" [Copy-Safe]
---
...
> **Tip:** If you don't specify a model, primary agents use the [model globally configured](https://opencode.ai/docs/config#models) while subagents will use the model of the primary agent that invoked the subagent.
